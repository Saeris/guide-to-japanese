...
import type { Data, Parent, ListContent, Root, Text } from "mdast";
...
type TagListData = Data;

interface TagList extends Parent {
  type: `taglist`;
  children: ListContent[];
  data?: TagListData | undefined;
}
...
export const REGEX = /\{\|([\w-]+(?:,\s*[\w-]+)*)\|\}/;
export const REGEX_GLOBAL = /\{\|([\w-]+(?:,\s*[\w-]+)*)\|\}/g;
...
export const plugin: Plugin<void[], Root> = () => {
   ...
   */
  const constructQuotationNode = (children: ListContent[]): TagList => ({
    type: `taglist`,
    children,
    data: {
      hName: `ul`,
  ...
  const visitorSecond: Visitor<Text, Parent> = (
    ...
    const beforeChildren = findAllBefore(parent, openingNode) as ListContent[];
    const mainChildren = findAllBetween(
      parent,
      openingNode,
      closingNode
    ) as ListContent[];
    const afterChildren = findAllAfter(parent, closingNode) as ListContent[];
